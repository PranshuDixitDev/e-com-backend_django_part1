...
class BulkUploadModelTest(TestCase):
    ...
    def test_bulk_upload_str_representation(self):
        ...
        expected_str = f"Bulk Upload {bulk_upload.id} - {bulk_upload.get_status_display()}"
...
class BulkUploadAdminTest(TestCase):
    ...
    def test_admin_readonly_fields(self):
        ...
        expected_fields = [
            ...
            'processing_results_summary', 'formatted_category_stats', 
            'formatted_detailed_errors', 'formatted_empty_categories'
        ]
...
class BulkUploadIntegrationTest(TestCase):
    """Integration tests for BulkUpload functionality."""
    ...
    def test_bulk_upload_model_integration(self):
        """Test BulkUpload model integration with other components."""
        zip_file = SimpleUploadedFile(
            "integration_test.zip",
            b"test content for integration",
            content_type="application/zip"
        )
        ...
        # Test model creation
        self.assertIsNotNone(bulk_upload.id)
        self.assertEqual(bulk_upload.status, 'pending')
        ...
        updated_upload = BulkUpload.objects.get(id=bulk_upload.id)
        self.assertEqual(updated_upload.status, 'completed')
        
    def test_bulk_upload_admin_integration(self):
        """Test BulkUpload admin integration."""
        from django.contrib.admin.sites import site
        
        # Check if BulkUpload is registered in admin
        self.assertIn(BulkUpload, site._registry)
        
        # Get the admin instance
        admin = site._registry[BulkUpload]
